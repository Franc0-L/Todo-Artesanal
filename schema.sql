...
create or replace view vista_pedidos_semana as
select
  p.id as pedido_id,
  c.id as cliente_id,
  c.nombre,
...
-- SEGURIDAD (RLS)
-- ============================================================
-- El panel de administración usa un usuario autenticado de Supabase (el de tu mamá)
-- y tiene acceso completo a las tablas.
...
create policy "admin_full_access_clientes" on clientes for all to authenticated using (true) with check (true);
create policy "admin_full_access_semanas" on semanas for all to authenticated using (true) with check (true);
create policy "admin_full_access_dias_menu" on dias_menu for all to authenticated using (true) with check (true);
create policy "admin_full_access_pedidos" on pedidos for all to authenticated using (true) with check (true);
...
begin
  ...
  select id into v_cliente_id from clientes where token = p_token and activo = true;
  if v_cliente_id is null then
    raise exception 'Cliente no encontrado';
  end if;

  insert into pedidos (cliente_id, dia_menu_id, tipo_menu)
  values (v_cliente_id, p_dia_menu_id, p_tipo)
