...
CREATE UNIQUE INDEX week_day_options_week_day_offer_modality_unique
  ON public.week_day_options (week_day_id, offer_modality);

-- Activación: cada día debe tener exactamente una oferta General y una Opcional.
...
BEGIN
  ...
  WHERE wd.week_id = p_week_id
    AND (
      NOT EXISTS (SELECT 1 FROM public.week_day_options wdo WHERE wdo.week_day_id = wd.id AND wdo.offer_modality = 'general')
      OR NOT EXISTS (SELECT 1 FROM public.week_day_options wdo WHERE wdo.week_day_id = wd.id AND wdo.offer_modality = 'opcional')
    );
  ...
  WHERE (
    SELECT count(*) FROM public.menu_version_items mvi
    WHERE mvi.menu_version_id = used_menus.menu_version_id AND mvi.role = 'main'
  ) <> 1;
...
-- Pedido manual: General/Opcional debe coincidir con la oferta elegida.
...
BEGIN
  ...
  IF new.modality IN ('general', 'opcional') AND new.modality <> v_offer_modality THEN
    RAISE EXCEPTION 'La opción seleccionada pertenece a la oferta %, no a la modalidad %', v_offer_modality, new.modality;
  END IF;
