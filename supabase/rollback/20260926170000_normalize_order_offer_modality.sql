-- Rollback: restaurar validate_order del estado inmediatamente anterior.
-- Después de este rollback, General/Opcional vuelven a validarse contra
-- la modalidad enviada por la UI.
...
BEGIN
  SELECT wd.week_id, wdo.offer_modality INTO v_week_id, v_offer_modality
  FROM public.week_day_options wdo JOIN public.week_days wd ON wd.id = wdo.week_day_id
