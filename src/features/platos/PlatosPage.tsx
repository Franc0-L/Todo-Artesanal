...
export function PlatosPage() {
  ...
  const handleCloseDrawer = useCallback(() => { setSelectedDishId(null); setCreateDrawerOpen(false); }, []);
  const handleDishCreated = useCallback((created: Dish) => { setCreateDrawerOpen(false); setSelectedDishId(created.id); setPage(1); }, []);
  ...
  useEffect(() => {
    ...
    filterDebounceRef.current = window.setTimeout(() => { setPage(1); setSearch(searchInput.trim()); setCategory(categoryInput.trim()); }, SEARCH_DEBOUNCE_MS);
  ...
  function handleSearchSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    setPage(1); setSearch(searchInput.trim()); setCategory(categoryInput.trim());
  }
  ...
  return (
    <section className="platos-page" aria-labelledby="platos-title">
      <header className="platos-page__header"><div className="platos-page__heading-row"><div><p className="platos-page__eyebrow">Administración</p><h1 id="platos-title">Platos</h1><p className="platos-page__description">Gestioná el catálogo de platos y sus versiones. Editar nombre o precio crea una versión nueva; la anterior queda intacta.</p></div><button className="platos-primary-action" type="button" onClick={() => { setSelectedDishId(null); setCreateDrawerOpen(true); }}>Nuevo plato</button></div></header>
      <div className="platos-toolbar">
        <form className="platos-search" onSubmit={handleSearchSubmit}><div className="platos-search__field"><label htmlFor="dish-search">Buscar</label><input id="dish-search" name="search" type="search" value={searchInput} onChange={(event) => setSearchInput(event.target.value)} placeholder="Nombre del plato" /></div><div className="platos-search__field"><label htmlFor="dish-category">Categoría</label><input id="dish-category" name="category" type="text" value={categoryInput} onChange={(event) => setCategoryInput(event.target.value)} placeholder="Ej: Principal" /></div><button type="submit">Buscar</button></form>
      ...
      <div className="platos-list-wrapper" aria-busy={loading}>
        {loading ? <p className="platos-feedback">Cargando platos…</p> : items.length === 0 ? <div className="platos-feedback"><h2>No hay platos para mostrar</h2><p>{hasActiveFilters ? "Probá cambiar la búsqueda o los filtros." : "Todavía no hay platos registrados."}</p></div> : <><div className="platos-list-header" aria-hidden="true"><span>Nombre</span><span>Categoría</span><span>Clima</span><span>Estado</span></div><ul className="platos-list" aria-label="Listado de platos">{items.map((dish) => <li key={dish.id}><button className="dish-row" type="button" onClick={() => setSelectedDishId(dish.id)} aria-label={`Abrir ficha de ${dish.name ?? "plato sin versión"}`}><strong>{dish.name ?? "Sin nombre"}</strong><span>{dish.category ?? "—"}</span><span>{dish.climate ? CLIMATE_LABELS[dish.climate] : "Sin preferencia"}</span><span><span className={`platos-status platos-status--${dish.active ? "active" : "inactive"}`}>{dish.active ? "Activo" : "Inactivo"}</span></span></button></li>)}</ul></>}
      </div>
