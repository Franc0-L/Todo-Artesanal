...
create unique index una_semana_activa on semanas (activa)
where
  activa = true;
...
create trigger trg_pedidos_actualizado before
...
create or replace function private.es_admin () returns boolean security definer
set
  search_path = '' stable language sql as $$
  select exists (
    select 1
...
revoke all on function private.es_admin ()
from
  public;
...
create policy "admin_full_access_clientes" on clientes for all to authenticated using (
  (
    select
      private.es_admin ()
  )
...
create policy "admin_full_access_semanas" on semanas for all to authenticated using (
  (
    select
      private.es_admin ()
  )
...
create policy "admin_full_access_dias_menu" on dias_menu for all to authenticated using (
  (
    select
      private.es_admin ()
  )
...
create policy "admin_full_access_pedidos" on pedidos for all to authenticated using (
  (
    select
      private.es_admin ()
  )
...
semanas,
dias_menu,
pedidos to authenticated;

revoke all on vista_pedidos_semana
from
  public,
  anon;
...
grant
select
  on vista_pedidos_semana to authenticated;
...
create or replace function submit_order (p_token text, p_dia_menu_id uuid, p_tipo text) returns void security definer
set
...
revoke all on function submit_order (text, uuid, text)
from
  public;
...
grant
execute on function submit_order (text, uuid, text) to anon;